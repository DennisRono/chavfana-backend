...
from chavfana.schemas.user import UserCreate, UserRead, UserUpdate, EmployeeCreate, EmployeeRead
from chavfana.core.exceptions import NotFoundError, BusinessLogicError, AuthenticationError
...
async def login(
    ...
    return await auth_controller.login(db=db, email=email, password=password)
...
async def get_users_by_role(
    ...
    return await auth_controller.get_users_by_role(db=db, role=role, skip=skip, limit=limit)
...
async def update_user(
    ...
    return await auth_controller.update_user(db=db, user_id=user_id, request_data=request_data)
...
@auth_router.post("/employees", response_model=EmployeeRead, status_code=status.HTTP_201_CREATED)
...
async def get_employees_by_farm(
    farm_id: UUID,
    db: AsyncSession = Depends(get_db)
):
...
async def update_employee(
    ...
    return await auth_controller.update_employee(db=db, employee_id=employee_id, request_data=request_data)
